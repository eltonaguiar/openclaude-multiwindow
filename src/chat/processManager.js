...
class ProcessManager {
  ...
  _onStderr(chunk) {
    ...
    // Demote known non-fatal CLI warnings to a status message so they don't
    // surface as red error toasts. The "[context] Warning: model ... not in
    // context window table" line is emitted whenever an unknown model id is
    // passed via --model (e.g. via the OpenAI shim with a custom model).
    // Users can silence it by adding the model to openclaude.modelContextOverrides.
    for (const line of trimmed.split(/\r?\n/)) {
      const t = line.trim();
      if (!t) continue;
      if (/\[context\]\s+Warning:/i.test(t)) {
