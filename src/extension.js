...
const REGISTRY_STATE_KEY = 'openclaude.multichat.v1';
...
function activate(context) {
  ...
  const chatRegistry = new ChatRegistry(sessionManager);

  // Restore persisted chats (metadata only; sessions reload on demand).
  try {
    const snapshot = context.globalState.get(REGISTRY_STATE_KEY, null);
    if (snapshot) chatRegistry.restore(snapshot);
  } catch (err) {
    console.warn('[openclaude] failed to restore chat registry:', err && err.message);
  }

  const chatPanelManager = new OpenClaudeChatPanelManager(chatRegistry);
  const chatViewProvider = new OpenClaudeChatViewProvider(chatRegistry, chatPanelManager);

  // Persist on every registry change (debounced).
  ...
  const persistRegistry = () => {
    ...
    persistTimer = setTimeout(() => {
      persistTimer = null;
      try {
        context.globalState.update(REGISTRY_STATE_KEY, chatRegistry.serialize());
      } catch (err) {
        console.warn('[openclaude] failed to persist chat registry:', err && err.message);
      }
    }, 250);
  };
  const registryPersistSub = chatRegistry.onDidChange(persistRegistry);
  const persistTimerDisposer = { dispose: () => { if (persistTimer) { clearTimeout(persistTimer); persistTimer = null; } } };
  ...
  const refreshStatusBar = () => {
    const list = chatRegistry.list();
    const streaming = list.filter(c => c.isStreaming);
    if (streaming.length > 0) {
      statusBarItem.text = `$(sync~spin) OpenClaude (${streaming.length})`;
      statusBarItem.tooltip = `OpenClaude generating in ${streaming.length} chat(s)`;
    } else {
      statusBarItem.text = `$(comment-discussion) OpenClaude (${list.length})`;
      statusBarItem.tooltip = `${list.length} chat(s) open`;
    }
  };
  const statusBarSub = chatRegistry.onDidChange(refreshStatusBar);
  refreshStatusBar();
  ...
  // ── New multi-chat commands ──
  const newChatCommand = vscode.commands.registerCommand('openclaude.newChat', () => {
    try {
      const id = chatRegistry.create({});
      chatRegistry.setActive(id);
      void vscode.commands.executeCommand('workbench.view.extension.openclaude');
    } catch (err) {
      void vscode.window.showWarningMessage(err.message || String(err));
    }
  });

  const openChatCommand = vscode.commands.registerCommand('openclaude.openChat', () => {
    const activeId = chatRegistry.activeId;
    if (activeId) {
      chatPanelManager.openPanelForChat(activeId);
    } else {
      const id = chatRegistry.create({});
      chatRegistry.setActive(id);
      chatPanelManager.openPanelForChat(id);
    }
  });

  const openChatInNewTabCommand = vscode.commands.registerCommand(
    'openclaude.openChatInNewTab',
    async (chatIdArg) => {
      let chatId = chatIdArg;
      if (!chatId) chatId = chatRegistry.activeId;
      if (!chatId) {
        const tabs = chatRegistry.list();
        if (tabs.length === 0) {
          chatId = chatRegistry.create({});
        } else {
          const picked = await vscode.window.showQuickPick(
            tabs.map(t => ({ label: t.title, description: t.model, chatId: t.id })),
            { placeHolder: 'Pick a chat to open in new editor tab' },
          );
          if (!picked) return;
          chatId = picked.chatId;
        }
      }
      chatPanelManager.openPanelForChat(chatId);
    },
  );
  ...
  const resumeSessionCommand = vscode.commands.registerCommand('openclaude.resumeSession', async () => {
    ...
    const picked = await vscode.window.showQuickPick(items, {
      placeHolder: 'Select a session to resume (creates a new chat)',
    });
    if (picked) {
      const id = chatRegistry.create({ title: picked.label, sessionId: picked.sessionId });
      const controller = chatRegistry.get(id);
      if (controller) {
        controller._currentSessionId = picked.sessionId;
        chatRegistry.setActive(id);
        await controller.startSession({ sessionId: picked.sessionId });
      }
    }
  });
